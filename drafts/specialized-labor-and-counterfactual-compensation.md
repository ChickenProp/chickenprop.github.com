...
I'm going to pretend that Cooperate and Defect are also called Work and Shirk, so that I don't have to distinguish them.
...
(How to read these symbols: the subscript is the player who gets the payoff, the first letter is their move, and the second letter is the other player's move. If the subscript is =, then this combination is symmetric. So `\( ws_1 \)` is the payoff to Player 1, if he Works while Player 2 Shirks. `\( ws_2 \)` is the payoff to Player 2, if she Works while Player 1 Shirks. `\( ws_= \)` is both of these values, when they're equal to each other. This is kind of experimental. Ellickson instead uses symbols `\( A, B, C, D, E \)` in descending order, but that makes it hard to remember which goes where in the grid, and when I extend it that ordering will be lost making it even more confusing.)
